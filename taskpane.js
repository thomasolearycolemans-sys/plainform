...
const DocumentAdapter = (function () {
  const HL = { swap: 'Turquoise', spell: 'Yellow', flag: 'BrightGreen' };
  ...
  function analyse(rawText, ignoreSet) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      if (merged.length && ABBREV_END.test(merged[merged.length - 1].text.trim())) {
        merged[merged.length - 1].text += chunks[i].text;
      } else {
    ...
    while ((nm = numRe.exec(masked)) !== null) {
      ...
      if (spanTaken(s, e)) continue;
      if (UNIT_WORDS.has(nm[2].toLowerCase())) continue;
...
/* ==================================================================== *
 *  PART 2b — CopilotModule : the OPTIONAL AI assistant (Gemini via relay).*
 * ==================================================================== */
const CopilotModule = (function () {
  // Prefer PLAINFORM_CONFIG.aiRelayUrl (set in config.js) so the relay
  // endpoint can be changed per-deployment without editing this file.
  // Falls back to the placeholder below, which keeps the module in safe
  // DEMO mode (no network call) exactly as before if nothing is configured.
  const cfg = window.PLAINFORM_CONFIG || {};
  const RELAY_URL = cfg.aiRelayUrl || 'https://REPLACE-WITH-YOUR-WORKER.workers.dev/review';

  let ENABLED = false;
  function setEnabled(on) { ENABLED = !!on; }
  function isEnabled() { return ENABLED; }
  function isLive() { return !!RELAY_URL && RELAY_URL.indexOf('REPLACE') === -1; }
...
function startPlainform() {
  ...
  // Visibility helpers: toggle the `.is-hidden` class instead of writing
  // inline style.display directly, so CSS owns all presentation (see
  // styles.css) and JS only ever adds/removes this one class.
  // editorPanel/aiPanel already carry `.panel-flex` in the markup, which
  // supplies the flex layout their internal .scroll area needs for a
  // bounded, scrollable height — see the .panel-flex/.scroll comment in
  // styles.css for why that matters.
  ...
  hide($('boot'));
  ['controls','legend','status','tabsRow'].forEach(id => show($(id)));
  show($('editorPanel'));
  ...
  function renderSuggestions(suggestions) {
    sugList.innerHTML = '';
    suggestions.length ? show(applyAllBtn) : hide(applyAllBtn);
  ...
  function afterSugChange() {
    ...
    remaining ? show(applyAllBtn) : hide(applyAllBtn);
    if (remaining === 0) {
      show(emptyState);
  ...
  function renderFlags(flags) {
    flagList.innerHTML = '';
    flags.length ? show(flagSection) : hide(flagSection);
  ...
  const tabs = [tabEditor, tabAI, tabHelp];
  function showTab(which) {
    ...
    which === 'editor' ? show(editorPanel) : hide(editorPanel);
    which === 'ai' ? show(aiPanel) : hide(aiPanel);
    which === 'help' ? show(helpPanel) : hide(helpPanel);
  }
  const TAB_WHICH = { tabEditor: 'editor', tabAI: 'ai', tabHelp: 'help' };
  tabs.forEach((tab, i) => {
    tab.addEventListener('click', () => { showTab(TAB_WHICH[tab.id]); tab.focus(); });
    tab.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowRight') { const t = tabs[(i + 1) % tabs.length]; showTab(TAB_WHICH[t.id]); t.focus(); }
      if (e.key === 'ArrowLeft') { const t = tabs[(i - 1 + tabs.length) % tabs.length]; showTab(TAB_WHICH[t.id]); t.focus(); }
    });
  });
  ...
  applyTheme(); // always sync data-theme + aria-pressed explicitly, even in light mode
  themeBtn.addEventListener('click', () => { dark = !dark; applyTheme(); });

  // Support contact in the Help panel is populated from config.js rather
  // than hard-coded in the HTML, so it can change per-deployment.
  const supportLink = $('supportLink');
  if (supportLink) {
    const cfg = window.PLAINFORM_CONFIG || {};
    supportLink.textContent = cfg.supportContactName || 'support';
    supportLink.href = cfg.supportContactHref || '#';
  }
